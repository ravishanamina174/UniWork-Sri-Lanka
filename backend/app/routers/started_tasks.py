...
async def verify_location(payload: VerifyLocationRequest):
    """
    Checks if student's current coordinates are within 10 meters 
    ...
    # 10 Meters Threshold Check
    RADIUS_THRESHOLD = 10.0

    if distance > RADIUS_THRESHOLD:
        raise HTTPException(
            status_code=400,
            detail=f"Location verification failed. You are currently {round(distance, 1)}m away. You must be within 10 meters of the pinned task location."
        )
