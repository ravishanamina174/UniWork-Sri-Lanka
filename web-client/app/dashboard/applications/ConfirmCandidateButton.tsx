...
export default function ConfirmCandidateButton({
  ...
  const handleConfirm = async () => {
    ...
    try {
      const res = await fetch(
        `/api/v1/applications/${applicationId}/confirm`,
        {
          method: "PATCH",
  ...
  if (status === "approve") {
    return (
      <button
        disabled
        className="w-full shrink-0 bg-emerald-50 border border-emerald-300 text-emerald-600 text-[13px] font-semibold py-1.5 px-4 rounded-[8px] transition-all duration-200 tracking-wide flex items-center justify-center gap-1.5 cursor-default"
      >
